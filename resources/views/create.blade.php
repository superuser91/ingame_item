<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route(config('vgp_ingame_item.routes.post_store_item')) }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" min="0" class="form-control @error('game_id') is-invalid @enderror" id="game_id"
                    name="game_id" aria-describedby="game_id" placeholder="GAME ID" value="{{ old('game_id') }}"
                    required>
                @error('game_id')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
